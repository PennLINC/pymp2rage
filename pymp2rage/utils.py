...
def MPRAGEfunc_varyingTR(
    ...
    MZsteadystate = 1.0 / (
        1 + inversionefficiency * (np.prod(cosalfaE1)) ** (nZslices) * np.prod(E_TD)
    )

    MZsteadystatenumerator = M0 * (1 - E_TD[0])

    for i in np.arange(nimages):
        MZsteadystatenumerator = MZsteadystatenumerator * cosalfaE1[i] ** nZslices + M0 * (
            1 - E_1[i]
        ) * (1 - (cosalfaE1[i]) ** nZslices) / (1 - cosalfaE1[i])
    ...
    m = 0
    temp = (-inversionefficiency * MZsteadystate * E_TD[m] + M0 * (1 - E_TD[m])) * (
        cosalfaE1[m]
    ) ** (nZ_bef) + M0 * (1 - E_1[m]) * (1 - (cosalfaE1[m]) ** (nZ_bef)) / (1 - (cosalfaE1[m]))

    signal[0] = sinalfa[m] * temp

    for m in range(1, nimages):
        temp = temp * (cosalfaE1[m - 1]) ** (nZ_aft) + M0 * (1 - E_1[m - 1]) * (
            1 - (cosalfaE1[m - 1]) ** (nZ_aft)
        ) / (1 - (cosalfaE1[m - 1]))

        temp = (temp * E_TD[m] + M0 * (1 - E_TD[m])) * (cosalfaE1[m]) ** (nZ_bef) + M0 * (
            1 - E_1[m]
        ) * (1 - (cosalfaE1[m]) ** (nZ_bef)) / (1 - (cosalfaE1[m]))
...
def MP2RAGE_lookuptable(
    ...
    Intensity = np.squeeze(
        ...
        / (np.abs(Signal[..., 0]) ** 2 + np.abs(Signal[..., 1]) ** 2)
    )
