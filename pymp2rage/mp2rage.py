...
from .utils import MP2RAGE_lookuptable, split_filename
...
class MP2RAGE:
    ...
    ----------
    MPRAGE_tr : float
        MP2RAGE TR in seconds
    invtimesAB : list of floats
        Inversion times in seconds
    flipangleABdegree : list of floats
        Flip angle of the two readouts in degrees
    nZslices : list of integers
        Slices Per Slab * [PartialFourierInSlice-0.5  0.5]
    FLASH_tr : float
        TR of the GRE readout
    sequence : string
        Kind of sequence (default is 'normal')
    inversion_efficiency : float
        Inversion efficiency of the MP2RAGE PULSE
        (Default is 0.96, as measured on a Siemens system).
    B0 : float
        Field strength in Tesla
    inv1_combined : filename or Nifti1Image, optional
        Magnitude and phase image corresponding to first inversion pulse.
        Should always consist of two volumes.
    inv2_combined : filename or Nifti1Image, optional
        Magnitude and phase image corresponding to second inversion pulse.
        Should always consist of two volumes.
    inv1 : filename or Nifti1Image, optional
        Magnitude image of first inversion pulse. Should always consist of one volume.
    inv1ph : filename or Nifti1Image, optional
        Phase image of first inversion pulse. Should always consist of one volume.
    inv2 : filename or Nifti1Image, optional
    ...
    t1map : Nifti1Image
        Quantitative T1 map
    t1w_uni : Nifti1Image
        Bias-field corrected T1-weighted image
    t1map_masked : Nifti1Image
    ...
    def fit_t1w_uni(self):
        ...
        self._t1w_uni = nb.Nifti1Image(self._t1w_uni, self.inv1.affine, self.inv1.header)

        return self._t1w_uni

    def fit_t1(self):
        if (
            (self.MPRAGE_tr is None)
            or (self.invtimesAB is None)
            or (self.flipangleABdegree is None)
            or (self.nZslices is None)
            or (self.FLASH_tr is None)
        ):
            raise Exception(
                'All sequence parameters (MPRAGE_tr, invtimesAB, flipangleABdegree, nZslices, '
        ...
        self._t1map = np.interp(-0.5 + self.t1w_uni.get_fdata() / 4096, Intensity, T1Vector)
        ...
        # Make image
        self._t1map = nb.Nifti1Image(self._t1map, self.t1w_uni.affine, self.t1w_uni.header)
    ...
    def fit_mask(self, modality='INV2', smooth_fwhm=2.5, threshold=None, **kwargs):
        ...
        if threshold is None:
            ...
            self._mask = image.math_img(f'im > {threshold}', im=im)
    ...
    def mask(self):
        if self._mask is None:
            logging.warning(
                'Mask is not computed yet. Computing the mask now with'
    ...
    def from_bids(
        ...
        layout = BIDSLayout(
            source_dir, validate=False, config=op.join(__dir__, 'bids', 'bep001.json')
        )
        ...
        invtimesAB = (
            df.loc[('MP2RAGE', 1, 'mag'), 'InversionTime'],
            df.loc[('MP2RAGE', 2, 'mag'), 'InversionTime'],
        )
        nZslices = df.loc[('MP2RAGE', 1, 'mag'), 'NumberShots']
        FLASH_tr = (
            df.loc[('MP2RAGE', 1, 'mag'), 'RepetitionTimeExcitation'],
            df.loc[('MP2RAGE', 2, 'mag'), 'RepetitionTimeExcitation'],
        )
        B0 = df.loc[('MP2RAGE', 1, 'mag'), 'FieldStrength']
        flipangleABdegree = (
            df.loc[('MP2RAGE', 1, 'mag'), 'FlipAngle'],
            df.loc[('MP2RAGE', 2, 'mag'), 'FlipAngle'],
        )
    ...
    def write_files(self, path=None, prefix=None, compress=True, masked=False):
        ...
        print(f'Writing T1 map to {t1map_filename}')
        print(f'Writing UNI T1-weighted image to {t1w_uni_filename}')
    ...
    def plot_B1_effects(self):
        ...
        def _signalres(x1, x2):
            return x1 * x2 / (x2**2 + x1**2)

        def _noiseres(x1, x2):
            return ((x2**2 - x1**2) ** 2 / (x2**2 + x1**2) ** 3) ** (0.5)
        ...
        for B1 in B1range:
            ...
            MP2RAGEamp, T1vector, IntensityBeforeComb = MP2RAGE_lookuptable(
                self.MPRAGE_tr,
                self.invtimesAB,
                effective_flipangle,
                self.nZslices,
            ...
            Signal = _signalres(
                IntensityBeforeComb[[posWM, posGM, posCSF], 0],
                IntensityBeforeComb[[posWM, posGM, posCSF], 1],
            )
            noise = _noiseres(
                IntensityBeforeComb[[posWM, posGM, posCSF], 0],
                IntensityBeforeComb[[posWM, posGM, posCSF], 1],
            ...
            Contrast.append(
                1000
                * np.sum((Signal[1:] - Signal[:-1]) / np.sqrt(noise[1:] ** 2 + noise[:-1] ** 2))
                / np.sqrt(self.MPRAGE_tr)
            )
        ...
        plt.text(0, (T1CSF + T1GM) / 2, '\n'.join([f'{c:.2f}' for c in Contrast]), va='center')
    ...
    def correct_for_B1(self, B1=None, check_B1_range=True):
        ...
        ----------
        B1 : filename
            B1 field map, either as a ratio or as a percentage. If
            set to None, use self.B1 (set when the MP2RAGE class was
            initialized).
        check_B1_range : bool
            whether the fuction should check whether the range
            of the B1 fieldmap makes sense (centered at 1, range of roughly 0-2 or 0-200).

        Returns
        -------
        tuple
            t1w_uni_b1_corrected : Nifti1Image
                A T1-weighted image corrected for B1 inhomogenieties
            t1map_b1_corrected : Nifti1Image
                A quantiative T1-weighted image corrected for B1 inhomogenieties
        """

        if B1 is None:
            if not hasattr(self, 'b1'):
                raise ValueError('Can not correct for B1 inhomogenieties without B1fieldmap')
            B1 = self.b1
        else:
            B1 = nb.load(B1)
            self.b1 = B1
        ...
        if B1.shape != self.inv1.shape:
            logging.warning(
                'B1 map has different resolution from '
        ...
        if check_B1_range:
            if np.median(B1.get_fdata()) > 10:
                logging.warning(
                    'B1 does not seem to be in range of 0-2. '
                    'Assuming B1 is measured as a percentage, dividing by '
                    '100...'
                )
                B1 = image.math_img('B1 / 100.', B1=B1)

            if np.median(B1.get_fdata()) > 2:
                raise ValueError(
                    'Median of B1 is far from 1. The scale of this B1 map is most likely wrong.'
                )
        ...
        y = self.t1w_uni.get_fdata() / 4095 - 0.5
        ...
        self.t1map_b1_corrected = nb.Nifti1Image(t1c * 1000, self.t1map.affine, self.t1map.header)
        ...
        t1w_uni_corrected = (f(t1c) + 0.5) * 4095
        self.t1w_uni_b1_corrected = nb.Nifti1Image(
            t1w_uni_corrected,
            self.t1w_uni.affine,
            self.t1w_uni.header,
        )
...
class MEMP2RAGE(MP2RAGE):
    ...
    def __init__(
        ...
        if self.t2starw_echoes.shape[-1] != self.n_echoes:
            raise ValueError(
                'Length of echo_times should correspond to the number of echoesin INV2'
            )
    ...
    def fit_t2star(self, min_t2star=0, max_t2star=300):
        tmp = np.log(self.t2starw_echoes.get_fdata())
        idx = (tmp > 0).all(-1)
        ...
        x = np.concatenate(
            (np.ones((self.n_echoes, 1)), -self.inv2_echo_times[..., np.newaxis]), 1
        )

        beta, _, _, _ = np.linalg.lstsq(x, tmp[idx].T)

        s0[idx] = np.exp(beta[0])
        t2star[idx] = 1.0 / beta[1]
    ...
    def write_files(self, path=None, prefix=None, compress=True, *args, **kwargs):
        files = super().write_files(path, prefix, compress, *args, **kwargs)
        ...
        print(f'Writing T2 star-weighted image to {t2starw_filename}')
        ...
        print(f'Writing T2 star map to {t2starmap_filename}')
        ...
        print(f'Writing S0 map to {s0_filename}')
    ...
    def from_bids(
        ...
        MPRAGE_tr = df.loc[('MP2RAGE', 1, slice(None), 'mag'), 'RepetitionTimePreparation'].values[
            0
        ]
        invtimesAB = (
            df.loc[('MP2RAGE', 1, slice(None), 'mag'), 'InversionTime'].values[0],
            df.loc[('MP2RAGE', 2, slice(None), 'mag'), 'InversionTime'].values[0],
        )
        nZslices = df.loc[('MP2RAGE', 1, slice(None), 'mag'), 'NumberShots'].values[0]
        FLASH_tr = (
            df.loc[('MP2RAGE', 1, slice(None), 'mag'), 'RepetitionTimeExcitation'].values[0],
            df.loc[('MP2RAGE', 2, slice(None), 'mag'), 'RepetitionTimeExcitation'].values[0],
        )
        B0 = df.loc[('MP2RAGE', 1, slice(None), 'mag'), 'FieldStrength'].values[0]
        flipangleABdegree = (
            df.loc[('MP2RAGE', 1, slice(None), 'mag'), 'FlipAngle'].values[0],
            df.loc[('MP2RAGE', 2, slice(None), 'mag'), 'FlipAngle'].values[0],
        )
...
def _get_B1map(layout, subject, session):
    ...
    if len(B1_filenames) == 1:
        ...
        print(f'found one B1 map: {B1_fieldmap}')

    if len(B1_filenames) > 1:
        B1_fieldmap = B1_filenames[0]
        print(f'found multiple B1 maps: {B1_filenames}')
        print(f'Using {B1_fieldmap} as B1 map')
